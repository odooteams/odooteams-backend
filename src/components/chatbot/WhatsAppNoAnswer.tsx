...
interface WhatsAppNoAnswerProps {
  isRtl: boolean;
  onWhatsAppClick: () => void;
}

const WhatsAppNoAnswer: React.FC<WhatsAppNoAnswerProps> = ({ isRtl, onWhatsAppClick }) => {
  const { t } = useLanguage();
  ...
  return (
    <div className={`flex items-center gap-2 mt-2 ${isRtl ? 'justify-end' : 'justify-start'}`}>
      <img 
      ...
      <div className="bg-gray-100 rounded-lg px-3 py-2 max-w-[70%]">
        <div className="flex items-center gap-2">
          <p className="text-sm text-gray-600">
            {t('Contact us directly on WhatsApp:', 'تواصل معنا مباشرة على واتساب:')}
          </p>
          <Button
...
export default WhatsAppNoAnswer;
