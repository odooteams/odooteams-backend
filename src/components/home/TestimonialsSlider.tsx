  ...
  useEffect(() => {
    const getTestimonials = async () => {
      try {
        ...
        const mappedTestimonials: Testimonial[] = data.map((row, index) => ({
          id: index + 1,
          name: row.name || '',
          title: row.Title || '',
          reviews: row.reviews || '',
          stars: parseInt(row.stars || '5', 10),
                ...
                <CarouselItem key={testimonial.id} className="md:basis-1/2 lg:basis-1/3">
                  <Card className="h-full border border-gray-200 hover:border-odoo-magenta transition-colors animate-scale-in seo-highlight"
                          ...
                          <div className="flex mb-1">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star 
                                key={i} 
                                className="h-4 w-4" 
                                fill={i < testimonial.stars ? "#FFC107" : "none"} 
                                stroke={i < testimonial.stars ? "#FFC107" : "#CBD5E1"} 
                              />
                            ))}
                          </div>
                    ...
                    <CardContent className="py-4">
                      <p className="text-gray-700 mb-4 line-clamp-4">{testimonial.reviews}</p>
                    </CardContent>
